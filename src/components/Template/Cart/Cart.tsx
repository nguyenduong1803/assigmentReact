...
const Cart = (props: Props) => {
  ...
  return (
    <>
      <IconButton onClick={() => setOpen(true)} size='medium' sx={{ ml: 2 }} aria-haspopup='true'>
        ...
        <Box width='400px' height='100%' p={3}>
          <Box display='flex' justifyContent='space-between' flexDirection='column' height='100%'>
            ...
            <Box flex='auto' overflow='auto'>
              {cart ? <RenderCart /> : <p>Chưa có sản phẩm nào trong giỏ hàng</p>}
            </Box>
            <Typography textAlign='center' m={4} variant='h5'>
              {cart && (
                <ButtonLink onClick={() => setOpen(false)} variant='contained' to='/checkout'>
                  Checkout
                </ButtonLink>
              )}
            </Typography>

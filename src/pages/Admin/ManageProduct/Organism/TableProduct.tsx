...
export default function TableProduct() {
  ...
  const { isLoading, error, data, refetch } = useQuery('repoData', () => getProduct({ limit: 8 }), {
    staleTime: 4000
  });
  useEffect(() => {
    if (isUpdate) {
  ...
  const listRow =
    ...
    data.data.map((row: any, index: number) => (
      <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
        ...
        {open && <ModalDelete removeProduct={removeProduct} setOpen={setOpen} id={row._id} open={open} />}
      </TableRow>
    ));
